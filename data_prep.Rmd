...
# Introduction
In this blog I will walk through the steps I take in analyzing and modelling the data. This is the first Kaggle competition I participate in. The the competition [here](https://www.kaggle.com/c/house-prices-advanced-regression-techniques/overview). 

## First step
First I combine the training and the test set. I only do this because I fix few variable that have missing values but with know reason, see the data description text file. After fixing the missing values I split the dataset back to training and test using the id's from both dataset. Since the reason of missingness is known and the same between training and test set there is no danger of lookahead bias.  


# Missing data
I start by fixing the obvious missing values, see data secription text file. The only problematic variable seem to be the year build of garage. Further below in this document is analysis on what can be done regarding that variable.    
...
Using co-occurance plot can further deepen our understanding of missing information. With this plot we can see if any variables tend to be missing together. For your dataset that doesn't seem to be the case except maybe for mas_vnr_area and mas_vnr_type. Those are only 16 samples out of 2.919. Those will be imputed later using K-nearest neighbors (KNN).  
...
### lot_frontage
lot_frontage referce to *Linear feet of street connected to property*. When compared to SalePrice there doesn't seems to be that much of a difference in sale price whether or not the lot_frontage is missing. It could be that the actual value is 0 but we don't know for sure. We could also impute the missing values using KNN or Bagging and compare it to imputing the missing values with zero.  
...
Next I will train two random forest models to shed light on if I should imputed lot_frontage using KNN or with zeros. As will be discussed below, recoding garage_yr_blt as zero-one variable is better than omitting it, I will do it here when analyzing lot_frontage.  

```{r, lot_frontage_rf}
training <- all %>% filter(id %in% train_id)
training$sale_price <- sale_price

# Impute with zero
lot_zero <- training %>% 
        mutate(lot_frontage = case_when(is.na(lot_frontage) ~ 0,
                                        TRUE ~ lot_frontage),
               garage_yr_blt = case_when(is.na(garage_yr_blt) ~ 0,
                                         TRUE ~ 1)) %>% 
        mutate_if(is.character, as.factor)

lot_train_sample  <- sample(nrow(training), nrow(training)*2/3)
...
# Modelling
n_features_lot <- length(setdiff(names(training), "sale_price"))
...
rf_model_impute <- ranger::ranger(sale_price ~ .,
                               data = lot_impute_train[, -1],  # delet the id
                               mtry = floor(n_features_lot / 3),
                               respect.unordered.factors = "order",
                               importance = "impurity")
...
The difference in terms of RMSE on test set is really small and not conclusive. Thus I will use what is easier to use, i.e. imputation.  
...
recode_omit_ratio <- scales::percent(RMSE(pred_rec, train_gar_recode_test$sale_price) / RMSE(pred_omit, train_gar_omit_test$sale_price) - 1)

```

There is difference in RMSE with garage_yr_blt recoded giving higher accuracy than omitting it, the RMSE being `recoded_omit_ratio` higher with the variable recoded. This is the action taken here for this variable.  
